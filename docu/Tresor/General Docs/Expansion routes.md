...
- your-vps-provider VPS with WireGuard tunnel proxying external traffic into Tresor
- Full Ansible automation (~15-20 roles), getting unwieldy
- Cloud footprint: your-vps-provider VPS, Cloudflare DNS, AWS SES — all managed manually via UI

---

## Phase 1 — Terraform (first, any time)

**Goal:** Codify existing cloud infrastructure as source of truth and DR artifact.

**What to cover:**
- VPS (server, SSH key, firewall rules)
- Cloudflare DNS records and zones
- AWS SES verified identities
- Quiz app VPS (separate Terraform project/workspace)

**Why now:** If vps-provider dies tomorrow, currently have to remember everything manually. With Terraform it's `terraform apply` and Ansible handles the rest. Also small surface area — ~150-200 lines HCL total for current footprint. Easy win before the infra grows.

**Key concepts to learn as you go:**
- Providers (hcloud, cloudflare)
- Resources, variables, outputs
- Remote state backend (Terraform Cloud free tier or your-vps-provider Object Storage)
...
- Terraform provisions nodes if cloud (your-vps-provider), Ansible configures everything
...
```
Now          → Terraform for existing cloud infra (your-vps-provider, Cloudflare, SES)
